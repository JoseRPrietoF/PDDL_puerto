(define (domain puerto)
	...
	(:durative-action increaseSteps
		:parameters (?m - muelle)
		:duration (= ?duration 0)
        :condition (at start (>= (flag) 2))
        :effect 
	)
	...
	(:durative-action coger
        ...
        :condition (and (at start (> (steps) 0))
						(at start (< (flag) 2))
        ...
        :effect (and
				   ...
				   (at end(decrease (steps) 1 ))
				   (at end(increase (flag) 1 ));Variable para habilitar la regeneracion de steps
				)
	)
